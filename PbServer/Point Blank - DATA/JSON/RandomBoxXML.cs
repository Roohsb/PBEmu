    ...
    public class RandomBoxXML
    {
        ...
        private static void LoadBox(int id)
        {
            ...
            if (File.Exists(path))
                ...
                Logger.Error("[RandomBoxXML] There is no file: " + path);
        }
        private static void parse(string path, int cupomId)
        {
            ...
            using (FileStream fileStream = new FileStream(path, FileMode.Open))
            {
                if (fileStream.Length == 0)
                    Logger.Error("[RandomBoxXML] The file is empty: " + path);
                else
                {
                    try
                    {
        ...
        public static bool ContainsBox(int id) => boxes.ContainsKey(id);
        public static RandomBoxModel getBox(int id) => boxes[id];
    }
}