...
namespace Battle.data.xml
{
    public class MappingXML
    {
        ...
        {
            switch(obj._ultraSYNC)
            {
                case 0: break;
                case int sync_1 when (sync_1 == 1 || sync_1 == 3):
                    {
                        room._bar1 = obj._life;
                        room._default1 = room._bar1;
                        break;
                    }
                case int sync_2 when (sync_2 == 2 || sync_2 == 4):
                    {
        ...
        {
            string path = "data/battle/map.xml";
            if (File.Exists(path))
                parse(path);
            else
                Logger.Warning("[MappingXML] Não há nenhum arquivo: " + path);
        }
        private static void parse(string path)
        {
            ...
            using (FileStream fileStream = new FileStream(path, FileMode.Open))
            {
                            ...
                            if ("list".Equals(xmlNode1.Name))
                            {
                                for (XmlNode xmlNode2 = xmlNode1.FirstChild; xmlNode2 != null; xmlNode2 = xmlNode2.NextSibling)
                                {
                                    if ("Map".Equals(xmlNode2.Name))
                                    {
                                        ...
                                        {
                                            _id = int.Parse(xmlNode2.Attributes.GetNamedItem("Id").Value)
                                        };
                ...
                fileStream.Dispose();
                fileStream.Close();
                Logger.Carregar(" [XML  system] Foram Carregados " + _maps.Count + " Maps");
            }
        }
                    ...
                    for (XmlNode xmlNode4 = xmlNode3.FirstChild; xmlNode4 != null; xmlNode4 = xmlNode4.NextSibling)
                    {
                        if ("Obj".Equals(xmlNode4.Name))
                        {
                            ...
                            if (obj._anim1 > 255)
                            {
                                switch(obj._anim1)
                                {
                                    case 256: obj._ultraSYNC = 1; break;
                                    case 257: obj._ultraSYNC = 2; break;
                                    case 258: obj._ultraSYNC = 3; break;
                                    case 259: obj._ultraSYNC = 4; break;
                                    default: obj._anim1 = 255; break;
                                }                              
                            }
    ...
    public class ObjModel
    {
        ...
        public int GetARandomAnim(Room room, ObjectInfo obj)
        {
            if (_anims != null && _anims.Count > 0)
            {
                ...
                if (anim._otherObj > 0)
                {
                    GetAnim(anim._otherAnim, 0, 0, room._objects[anim._otherObj]);
                }
