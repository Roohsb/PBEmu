    ...
    public class DamageManager
    {
        ...
        public static void BoomDeath(Room room, Player pl, int weaponId, List<DeathServerData> deaths, List<ObjectHitInfo> objs, List<int> BoomPlayers)
        {
            ...
            for (int i = 0; i < BoomPlayers.Count; i++)
            {
                ...
                if (room.GetPlayer(slot, out Player player) && !player.isDead)
                {
        ...
        public static void SimpleDeath(List<DeathServerData> deaths, List<ObjectHitInfo> objs, Player killer, Player victim, int valor,int weapon, int hitPart, CHARA_DEATH deathType)
        {
            if (valor <= 0)
