...
namespace Game.data.model
{
    public class Room
    {
        ...
        public List<long> ReadysPvp = new List<long>();
        public List<SLOT> mvp = new List<SLOT>();
        public bool isRoomMute = false, isSalaBot;
        ...
        {
            UniqueRoomId = (uint)((Settings.serverId & 0xff) << 20 | (_channelId & 0xff) << 12 | _roomId & 0xfff);
        }
        ...
        public bool IsBotMode() => special == 6 || special == 8 || special == 9;
        ...
        /// </summary>
        /// <returns>Tempo em segundos</returns>
        public int GetInBattleTimeLeft() => ((GetTimeByMask() * 60) - GetInBattleTime());
        ...
        public void StartBomb()
        {
            ...
            {
                SendDebug.SendInfo("StartBomb: " + ex.ToString());
            }
        }
        public void StartVote()
        {
            ...
            {
                SendDebug.SendInfo("[Room.StartVote] " + ex.ToString());
                if (vote.Timer != null)
                    vote.Timer = null;
                votekick = null;
            }
        ...
        public void RoundRestart()
        {
            ...
            catch (Exception ex) { SendDebug.SendInfo("[Room.RoundRestart] " + ex.ToString()); }
        }
        ...
        public void StartBattle(bool updateInfo)
        {
            ...
            {
                List<Account> list = GetAllPlayers(SLOT_STATE.READY, 0);
        ...
        public CupomEffects CupomEffectsCheck(Account p)
        {
            int checkout = 0;
            CupomEffects flag = p.effects;
            if (name.ToLower().Contains("@camp") || name.ToLower().Contains(" @camp") || name.ToLower().Contains("@camp ") || name.ToLower().Contains("camp") || name.ToLower().Contains("@pvp "))
            {
                CupomFlag C4SpeedKit = CupomEffectManagerJSON.GetCupomEffect(1200034000);
                CupomFlag HP10 = CupomEffectManagerJSON.GetCupomEffect(1200028000);
                CupomFlag Colete10 = CupomEffectManagerJSON.GetCupomEffect(1200030000);
                CupomFlag Colete20 = CupomEffectManagerJSON.GetCupomEffect(1200079000);
                CupomFlag Colete90 = CupomEffectManagerJSON.GetCupomEffect(1200065000);
                if (C4SpeedKit != null && p.effects.HasFlag(C4SpeedKit.EffectFlag))
                    flag -= C4SpeedKit.EffectFlag;
                if (HP10 != null && p.effects.HasFlag(HP10.EffectFlag))
                    flag -= HP10.EffectFlag;
                if (Colete10 != null && p.effects.HasFlag(Colete10.EffectFlag))
                    flag -= Colete10.EffectFlag;
                if (Colete20 != null && p.effects.HasFlag(Colete20.EffectFlag))
                    flag -= Colete20.EffectFlag;
                if (Colete90 != null && p.effects.HasFlag(Colete90.EffectFlag))
                    flag -= Colete90.EffectFlag;
            }
            return flag;
        }
        public void StartCountDown()
        {
            ...
            countdown.StartJob(5250, (callbackState) =>
            {
                ...
                {
                    SendDebug.SendInfo("[Room.StartCountDown] " + ex.ToString());
                }
                lock (callbackState)
        ...
        private void BaseResultGame(TeamResultType winnerTeam, bool isBotMode)
        {
            ...
            bool mapEvUse = EventMapSyncer.EventIsValid(evMap, mapId, room_type);
            ...
            {
                SLOT slot = _slots[i];
                if (!slot.check && slot.state == SLOT_STATE.BATTLE && GetPlayerBySlot(slot, out Account player))
                {
                    ...
                    if (!isBotMode)
                    {
                        ...
                        if (room_type == 2 || room_type == 4)
                        {
                            ...
                            slot.money = ((int)((slot.Score / 5.0) + (timePlayed / 7.0) + (slot.allDeaths * 5.0) + (slot.objetivos * 30))) * 2;
                        }
                        else
                        {
                            ...
                            slot.money = ((int)((slot.Score / 5.0) + (timePlayed / 7.0) + (slot.allDeaths * 5.0) + (slot.objetivos * 30))) * 3;
                        }
                        if (MVP == player.player_name)
                        {
                            slot.exp = (int)(slot.Score + (timePlayed / 3.5) + (slot.allDeaths * 1.8) + (slot.objetivos * 20));
                            slot.money += 300;
                        }
                    ...
                    else
                    {
                        ...
                        if (level == 0)
                            level++;
                        int reward = (LorenstudioSettings.MultiExpBot * slot.Score / level);
                        slot.gp += reward;
                        slot.exp += reward;

                    }
                    slot.exp = slot.exp > Settings.maxBattleXP ? Settings.maxBattleXP : slot.exp;
                    slot.gp = slot.gp > Settings.maxBattleGP ? Settings.maxBattleGP : slot.gp;
                    slot.money = slot.money > Settings.maxBattleMY ? Settings.maxBattleMY : slot.money;
                    if (slot.exp < 0 || slot.gp < 0 || slot.money < 0)
                    {
                        SendDebug.SendInfo("[1]SLOT XP: " + slot.exp + "; GOLD: " + slot.gp + "; MONEY: " + slot.money);
                        SendDebug.SendInfo("InBattleTime: " + inBattleTime + " [Finish: " + finishDate.ToString("yyMMddHHmmss") + "; Start: " + slot.startTime.ToString("yyMMddHHmmss") + "]");
                        SendDebug.SendInfo("allDeaths: " + slot.allDeaths + "; Objs: " + slot.objetivos + "; IsBOT: " + isBotMode);
                        slot.exp = 2;
                        slot.gp = 2;
                        slot.money = 2;
                    }
                    ...
                    if (evUp != null || mapEvUse)
                    {
                        if (evUp != null)
                        {
                            pctgXP += evUp._percentXp;
                            pctgGP += evUp._percentGp;
                        }
                        if (mapEvUse)
                        {
                            pctgXP += evMap._percentXp;
                            pctgGP += evMap._percentGp;
                        }
                    ...
                    int valorVip2 = 0;
                    int valorVip3 = 0;
                    if (player.pc_cafe > 0 && !isBotMode)
                    {

                        switch ((int)player.pc_cafe)
                        {
                            case 1:
                                valorVip1 = LorenstudioSettings.Vip1Exp;
                                valorVip2 = LorenstudioSettings.Vip1Gold;
                                valorVip3 = LorenstudioSettings.Vip1Cash;
                                slot.exp += (int)(slot.Score + (3 / 2.2) + (slot.allDeaths * 1.2) + (slot.objetivos * 5));
                                break;
                            case 2:
                                valorVip1 = LorenstudioSettings.Vip2Exp;
                                valorVip2 = LorenstudioSettings.Vip2Gold;
                                valorVip3 = LorenstudioSettings.Vip2Cash;
                                slot.exp += (int)(slot.Score + (5 / 2.3) + (slot.allDeaths * 2.2) + (slot.objetivos * 10));
                                break;
                            case 5:
                                valorVip1 = LorenstudioSettings.Vip5Exp;
                                valorVip2 = LorenstudioSettings.Vip5Gold;
                                valorVip3 = LorenstudioSettings.Vip5Cash;
                                slot.exp += (int)(slot.Score + (7 / 2.4) + (slot.allDeaths * 3.2) + (slot.objetivos * 15));
                                break;
                            case 6:
                                valorVip1 = LorenstudioSettings.Vip6Exp;
                                valorVip2 = LorenstudioSettings.Vip6Gold;
                                valorVip3 = LorenstudioSettings.Vip6Cash;
                                slot.exp += (int)(slot.Score + (10 / 2.5) + (slot.allDeaths * 4.2) + (slot.objetivos * 20));
                                break;
                        }
                        pctgXP += valorVip1;
                        pctgGP += valorVip2;
                        player._money += valorVip3;
                        if ((int)player.pc_cafe <= 2 && !slot.bonusFlags.HasFlag(ResultIcon.Pc))
                            slot.bonusFlags |= ResultIcon.Pc;
                        else if ((int)player.pc_cafe >= 5 && (int)player.pc_cafe <= 6 && !slot.bonusFlags.HasFlag(ResultIcon.PcPlus))
                    ...
                    if (player.Daily != null)
                    {
                        player.Daily.Point += slot.gp + slot.BonusGP;
                        player.Daily.Exp += slot.exp + slot.BonusXP;
                        query2.AddQuery("point", player.Daily.Point);
                        query2.AddQuery("exp", player.Daily.Exp);
                    }
                    if (Settings.winCashPerBattle)
                        player._money += slot.money;

                    try
                    {
                        string[] pvp = name.Split(' ');
                        string comum = pvp[0];
                        int desafio = int.Parse(pvp[1]);
                        if (comum.ToLower().Contains("@pvp "))
                        {
                            bool WonTheMatch = (slot._team == (int)winnerTeam);
                            if (!WonTheMatch)
                                player._money -= desafio;
                            else
                                player._money += desafio;
                        }
                    }
                    ...
                    RankModel rank = RankJSON.GetRank(player._rank);

                    if (rank != null && player._exp >= (rank._onNextLevel + rank._onAllExp) && player._rank <= 50)
                    {
                        List<ItemsModel> items = RankJSON.GetAwards(player._rank);
                        if (items.Count > 0)
                    ...
                    if (Settings.winCashPerBattle && Settings.showCashReceiveWarn && !isBotMode)
                    {
                        int total_acm = slot.money + valorVip3;
                        string txtCash = " - CASH - \n Receive [" + slot.money + "], VIP [" + valorVip3 + "] \n Total [" + total_acm + "], End [" + player._money + "]";
                        player.SendPacket(new SERVER_MESSAGE_ANNOUNCE_PAK(txtCash));
                        string txtMvp = "MVP \n [" + MVP + "]";
                        player.SendPacket(new SERVER_MESSAGE_ANNOUNCE_PAK(txtMvp));
                    }
                }
            }
            UpdateSlotsInfo();
        ...
        public string CheckMvp()
        {
            ...
            {
                SLOT slot = _slots[i];
                if (slot.state == SLOT_STATE.BATTLE && GetPlayerBySlot(slot, out Account p) && p != null)
                {
                    if (slot.allKills > 0 && !fistcheck)
                    {
                        nickMvp = p.player_name;
                        killscount = slot.allKills;
                        fistcheck = true;
                    }
                    else if (slot.allKills > killscount)
                    {
                        nickMvp = p.player_name;
                        killscount = slot.allKills;
                    }
        ...
        public void ChanceDeDrop(Account p)
        {
            ...
            if (i > 55 && i < 60)
            {
                Message msg = new Message(15)
                {
                    ...
                    sender_id = 0,
                    text = "Parabéns você ganhou uma caixa de evento.",
                    state = 1
                };
                p.SendPacket(new INVENTORY_ITEM_CREATE_PAK(1, p, new ItemsModel(Settings.DropItem, ComDiv.GetItemCategory(Settings.DropItem), "DROP_ITEM", 1, 3)), false);
                p.SendPacket(new BOX_MESSAGE_RECEIVE_PAK(msg), false);
                MessageManager.CreateMessage(p.player_id, msg);
            }
        }
        public bool UpGeneral(RankModel rank, Account player, DateTime finishDate, DBQuery query)
        {
            try
            {
                if (player.IsGM())
                    return false;
                else if (player._rank < 48)
                    return false;
                else if (player._rank == 51)
                    return true;
                else
                {
                    for (int i = 0; i < RankJSON.Generais.Count; ++i)
                    {
                        RankModule module = RankJSON.Generais[i];
                        if (module.Player_Exp < player._exp)
                        {
                            Account DesRank = AccountManager.GetAccount(module.Player_id, false);
                            if (DesRank != null)
                            {
                                DBQuery UpdateQuery = new DBQuery();
                                SendDebug.SendInfo("O Jogador foi rebaixado [" + DesRank.player_name + "].");
                                UpdateQuery.AddQuery("rank", DesRank._rank -= 1);
                                ComDiv.UpdateDB("accounts", "player_id", DesRank.player_id, UpdateQuery.GetTables(), UpdateQuery.GetValues());
                                if (DesRank._isOnline)
                                    DesRank.SendPacket(new BASE_RANK_UP_PAK(DesRank._rank, RankJSON.GetRank(DesRank._rank)._onNextLevel));
                            }
                            RankJSON.Generais.Clear();
                            RankJSON.GetGenerais();
                            Thread.Sleep(5);
                            if (rank != null && module.rank > player._rank)
                            {
                                List<ItemsModel> items = RankJSON.GetAwards(player._rank);
                                if (items.Count > 0)
                                    player.SendPacket(new INVENTORY_ITEM_CREATE_PAK(1, player, items));
                                ...
                                query.AddQuery("last_rankup_date", (long)player.LastRankUpDate);
                                query.AddQuery("rank", player._rank);
                                SendDebug.SendInfo("O Jogador upou e ultrapassou o primeiro colocado. [" + player.player_name + "]");
                                break;
                            }
                            return true;
                        }
        ...
        private void CalculateClanMatchResult(int winnerTeam)
        {
            ...
            {
                SLOT slot = _slots[i];
                if (slot.state == SLOT_STATE.BATTLE && GetPlayerBySlot(slot, out Account p))
                {
            ...
            for (int i = 0; i < list.Values.Count; i++)
            {
                ...
                RankModel rankModel = ClanRankJSON.GetRank(clan._rank);
        ...
        private void CalculateSpecialCM(Clan clan, int winnerTeam, int teamIdx)
        {
            ...
            if (winnerTeam == teamIdx)
            {
                ...
                SendDebug.SendInfo("Clan: " + clan._id + "; Earned Points: " + POINTS);
                clan._pontos += POINTS;
                SendDebug.SendInfo("Clan: " + clan._id + "; Final Points: " + clan._pontos);
            }
            else
            {
                if (clan._pontos == 0)
                {
                    SendDebug.SendInfo("Clan did not lose Points due to low score.");
                    return;
                }
                float morePoints;
                if (room_type == 1)
                ...
                SendDebug.SendInfo("Clan: " + clan._id + "; Losed Points: " + POINTS);
                clan._pontos -= POINTS;
                SendDebug.SendInfo("Clan: " + clan._id + "; Final Points: " + clan._pontos);
            }
        }
        ...
        /// </summary>
        /// <returns></returns>
        public bool IsStartingMatch() => _state > 0;
        /// <summary>
        ...
        /// </summary>
        /// <returns></returns>
        public bool IsPreparing() => (int)_state >= 2;
        ...
        public void SwitchNewSlot(List<SLOT_CHANGE> slots, Account p, SLOT old, int teamIdx)
        {
            ...
            for (int i = 0; i < array.Length; i++)
            {
                ...
                if (slot._playerId == 0 && slot.state == 0 && p != null)
                {
                    slot.state = SLOT_STATE.NORMAL;
        ...
        public void UpdateSlotsInfo(CupomEffects cupom)
        {
            using ROOM_GET_SLOTINFO_PAK packet = new ROOM_GET_SLOTINFO_PAK(this, true, cupom);
            SendPacketToPlayers(packet);
        }
        /// <summary>
        ...
        public void SendPacketToPlayers(SendPacket packet, SendPacket packet2, SLOT_STATE state, int type)
        {
            ...
            if (players.Count == 0)
                return;
            byte[] data1 = packet.GetCompleteBytes("Room.SendPacketToPlayers(SendPacket,SendPacket,SLOT_STATE,int)-1");
        ...
        {
            if (player == null || !GetSlot(player._slotId, out SLOT slot))
                return;
        ...
        private void BaseRemovePlayer(Account player, SLOT slot, bool WarnAllPlayers, int quitMotive)
        {
            ...
            if (player != null && slot != null)
            {
                if ((int)slot.state >= 9)
                {
                    ...
                    if (quitMotive == 0 && name.ToLower().Contains("@pvp "))
                    {
                        int aposta = int.Parse(name.Split(' ')[1]);
                        player._money -= aposta;
                        DBQuery query = new DBQuery();
                ...
                player._room = null;
                SendDebug.SendInfo("saiu da sala '" + player.player_name + "'; slot_antigo: " + player._slotId + "; salaid: " + _roomId + "");
                player._slotId = -1;
            ...
            lock (_slots)
                for (int i = 0; i < 16; i++)
                {
                    ...
                    if (slot._playerId == 0 && slot.state == 0)
                    {
                        ...
                        p.UpdateCacheInfo();
                        SendDebug.SendInfo("entrou na sala'" + p.player_name + "'; addslot: " + p._slotId + "; roomid: " + _roomId + "");
                        return i;
                    }
                }
            return -1;
            ...
            lock (_slots)
                for (int i = 0; i < array.Length; i++)
                {
                    ...
                    if (slot._playerId == 0 && slot.state == 0 && p != null)
                    {
                        ...
                        slot.state = SLOT_STATE.NORMAL;
                        p._room = this;
                        SendDebug.SendInfo("entrou na sala do clã '" + p.player_name + "'; addslot: " + p._slotId + "; roomid: " + _roomId + "");
                        p._slotId = SlotIdx;
                        slot._equip = p._equip;
                        p._status.updateRoom((byte)_roomId);
                        AllUtils.SyncPlayerToClanMembers(p);
        ...
        private void BaseSpawnReadyPlayers(bool isBotMode)
        {
            ...
            using (BATTLE_RECORD_PAK packet3 = new BATTLE_RECORD_PAK(this))
            {
                ...
                {
                    SLOT slot = _slots[i];
                    if ((int)slot.state == 13 && slot.isPlaying == 1 && GetPlayerBySlot(slot, out Account player))
                    {
                        slot.isPlaying = 2;
                        if (_state == RoomState.PreBattle)
                        {
                        ...
                        else if (_state == RoomState.Battle)
                        {
                            using (BATTLE_STARTBATTLE_PAK packet4 = new BATTLE_STARTBATTLE_PAK(slot, player, dinos, isBotMode, false))
                                SendPacketToPlayers(packet4, SLOT_STATE.READY, 1);
                            if (room_type == 2 || room_type == 4)
                                Game_SyncNet.SendUDPPlayerSync(this, slot, 1);
                            else
                                player.SendCompletePacket(data);
                            player.SendCompletePacket(data2);
